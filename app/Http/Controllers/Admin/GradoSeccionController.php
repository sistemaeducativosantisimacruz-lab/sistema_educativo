...
{
    protected $gradoSeccionService;

    public function __construct(\App\Services\GradoSeccionService $gradoSeccionService)
    {
        $this->gradoSeccionService = $gradoSeccionService;
    }
    ...
    public function updateTutores(Request $request, GradoSeccion $gradoSeccione)
    {
        ...
        $nuevoCotutorId = $request->cotutor_id ? (int) $request->cotutor_id : null;

        try {
            $resultado = $this->gradoSeccionService->asignarTutoresYCursos($gradoSeccione, $nuevoTutorId, $nuevoCotutorId);
            return redirect()->route('admin.grado-secciones.index')->with('success', $resultado['mensaje']);
        } catch (\Exception $e) {
